...
const words = ref([]);
const prod_words = ref([]);
const selectWord = ref([]);
const name = ref("我是一段话，请对进行处理");
const keywordsRef = useTemplateRef("keywords");

const init = () => {
  const tempWords = prod_words.value.sort(
    (pre, curr) => pre.start_offset - curr.start_offset
  );
  prod_words.value = tempWords;
  words.value = [];
  if (tempWords.length === 0) {
    const obj = {
      end_offset: name.value.length,
      label: "8",
      prob: "1",
      start_offset: 0,
      word: name.value,
  ...
  for (let i = 0; i < tempWords.length; i++) {
    ...
    if (pre_end_offset !== start_offset) {
      const obj = {
        ...
        word: name.value.substring(pre_end_offset, start_offset),
        isWord: false,
      };
      total.push(obj);
    }
    ...
    if (i === tempWords.length - 1) {
      const obj = {
        ...
        word: name.value.substring(end_offset),
        isWord: false,
      };
      total.push(obj);
    }
...
const selectKey = (value) => {
  if (window.getSelection) {
    ...
    preSelectionRange.selectNodeContents(keywordsRef.value);
    ...
    const existWord = words.value.find(
...
const showSelectKey = () => {
  ...
  if (!selectWord.value.word) {
    console.log("没有选中的内容");
    return;
  }

  const { start_offset, end_offset } = selectWord.value;
  const temp = [];
  prod_words.value.map((item) => {
    if (item.end_offset <= start_offset || item.start_offset >= end_offset) {
      temp.push(item);
    }
  });

  temp.push(selectWord.value);

  prod_words.value = [...temp];
  init();
  // 获取推荐词列表

  selectWord.value = {}; // 清空选中词
  // 清除选中
...
const deleteWords = (word) => {
  prod_words.value.forEach((item, index) => {
    if (
      item.start_offset === word.start_offset &&
      item.end_offset === word.end_offset
    ) {
      prod_words.value.splice(index, 1);
    }
  });
...
onMounted(() => {
  ...
  init();
  document.body.onkeydown = quickSetKey;
});
